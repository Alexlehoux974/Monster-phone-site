...
async function listMonsterLED() {
  console.log('💡 PRODUITS LED MONSTER DANS SUPABASE\n');
  ...
  console.log(`✅ Catégories LED: ${ledCategories.map(c => c.name).join(', ')}\n`);
  ...
  if (!products || products.length === 0) {
    console.log('❌ Aucun produit LED MONSTER actif trouvé');
    return;
  }

  console.log(`📦 ${products.length} produits LED MONSTER actifs:\n`);
  ...
  console.log('='.repeat(80));
  console.log(`\n📊 Total: ${products.length} produits LED MONSTER\n`);
}
