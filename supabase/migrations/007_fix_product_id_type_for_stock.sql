-- ========================================
-- Fix Product ID Type for Stock Decrement
-- ========================================
-- Change function signature to accept TEXT for product_id
-- since order_items.product_id is TEXT, not UUID

-- Drop existing functions first
DROP FUNCTION IF EXISTS decrement_product_stock(UUID, INTEGER, UUID);
DROP FUNCTION IF EXISTS process_order_stock_decrement(UUID);

-- Modified function to decrement product variant stock using TEXT product_id
...
BEGIN
    ...
    WHERE id = v_product_uuid AND stock <= 0;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Stock decrement functions fixed to accept TEXT product_id!';
    RAISE NOTICE '✅ Compatible with order_items.product_id (TEXT type)';
END
$$;
